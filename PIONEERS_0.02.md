...
Combined doc: **Part 1** is the current session handoff (state of `obelisk.html`).
**Part 2** is the full build specification (`PIONEERS_0.1.md`).

---

# Part 1 — Session Context

Handoff notes for continuing work on `obelisk.html` in a new chat.

## What this project is

Browser-based isometric 2.5D action roguelite based on the spec in Part 2.
Title: **"Pioneers: Climbing the Obelisk"**. Single self-contained HTML file —
vanilla JS, Canvas 2D, SVG-style shape primitives, no external assets.

- Primary file: `c:/Users/robbc/Downloads/Exercise2Files/obelisk.html`
- Spec (embedded below as Part 2, originally at `c:/Users/robbc/Downloads/PIONEERS_0.1.md`)

## What's implemented (matches spec)

- **Title / class select / spell loadout** screens — title is "Pioneers: Climbing the Obelisk"
- **Fire and Ice classes** with the exact 10 spells each from the spec
  (Fireball, Flame Wave, Burning Dash, Inferno, Ember Shot, Heat Shield,
  Flame Pillar, Explosion Trap, Meteor Drop, Phoenix Surge / Ice Shard,
  Frost Nova, Ice Wall, Blizzard, Freeze Beam, Slow Field, Ice Dash,
  Spike Trap, Cryo Armor, Deep Freeze)
- **10 floors** with per-floor mechanics wired in `setupFloorObjective()`:
  - 1 Village Outskirts — 1-of-3 random quest (siege / hunt / search)
  - 2 Cave Maze — procedural, exit portal pre-spawned
  - 3 Mountain Descent — 5-minute timer, swarm spawns on expiry,
    Hobgoblin mini-boss at exit
  - 4 Cemetery Survival — 60s survive + final surge at 10s remaining,
    skeletons with re-assembly mechanic
  - 5 Tomb Trap Hallway — 60s door-closing timer + periodic fire trap bursts
  - 6 Safe Haven — non-combat screen, Merchant / Healer / Vendor cards
  - 7 Haunted Hotel — bellhops + servants
  - 8 Corrupted Forest — slimes split on death, random key drop + clear to finish
  - 9 Void — circular resonance meter, fills on kills, full heal on completion
  - 10 Wilhelm boss — 5 phases (80/60/40/20/0% HP), 3 attacks (spear dash,
    circular sweep, zigzag thrown spear), persistent damage zones each phase
- **Boon system** — 1-of-3 card pick after each floor except 6 and 10
  (per spec). 8 boons defined in `BOONS` array
- **Per-biome palettes** — `BIOMES` table, colors floor tiles + wall faces
  + sky + tint overlay per floor
- **Combat feel** — `triggerShake()` and `triggerHitPause()`, both consumed
  in `gameLoop()` and `renderFloor()`
- **Controls** — WASD move, LMB attack, Shift dash, Ctrl sprint (+50%),
  Space jump (dodges ground hazards via `jumpTimer`), Q/E/R/F spells, G interact
- **Expanded enemies** — goblin, orc, hobgoblin, troll, skeleton (reassembles),
  bellhop, servant, slime (splits into slimelets), demon, elf

## Recently fixed (last thing before context cutoff)

The player was nearly invisible and the camera didn't track properly.
Fixes applied:
- Added `snapCamToPlayer()` — sets camera directly from player world pos,
  called on every `loadFloor` / `loadBossFloor` / `loadSafeHaven` spawn
- In-game follow auto-snaps when player drifts >200px from center
  (sprint / dash was outrunning the lerp)
- Player sprite 50% bigger (body 14×20, head 10px), thick outline, direction
  indicator line pointing at the mouse, jump adds a visible vertical hop
- Walls shortened (28 → 20px), top face lightened so player reads clearly
- Wall tiles depth-sort slightly earlier than entities at the same grid cell,
  so the player is never hidden behind a wall at the same depth

## Known issues / likely next work

Based on the two screenshots the user sent after the camera fix:

1. **Floor 2 (Cave Maze) is too dark.** The biome sky `#080608` plus the
   `rgba(0,0,0,0.35)` overlay makes almost everything near-black. The
   player HP bar reads fine but the sprite itself is barely visible.
   Consider adding torch light pools (radial gradients) or bumping the
   overlay alpha down.

2. **Wall tile at same screen-space position as player still occludes
   slightly** in some corridor configurations. Depth offset of -0.1 for
   walls is small; if it still clips, bump to -0.3 or draw walls first
   entirely as a pre-pass.

3. **Boss zigzag spear** was simplified — the spec says it returns to
   Wilhelm after impact. Currently it just flies and expires. If the
   user wants the full behavior, add a return phase.

4. **Gear rarity** — spec defines Common/Uncommon/Rare/Legendary. Current
   `GEAR_POOL` uses `epic` as the top tier. Should rename to `legendary`
   and ensure Wilhelm's guaranteed drop is that tier.

5. **Relic system** — spec asks for 3-5 placeholder relics with passive
   effects. The Safe Haven vendor currently just grants a flat damage
   buff. Proper relic slots + the 4-relic inventory UI is not built.

6. **HUD polish** — spell icons show first 6 chars of name. Real SVG icons
   per spell would read better.

7. **Floor 6 Safe Haven rendering** — it's a fullscreen menu right now
   (no 3D scene). Spec calls for an isometric tavern interior. Works as
   a stub but not true to spec.

## Architecture quick-reference

Key functions to know:

- `loadFloor(n)` — dispatches to archetype + `setupFloorObjective(n, mapData)`.
  Floor 6 → `loadSafeHaven()`. Floor 10 → `loadBossFloor()`.
- `setupFloorObjective(n, mapData)` — sets up enemies, `G.objective`,
  and `G.floorState` per floor.
- `checkObjective()` — marks `G.objectiveDone` and spawns exit portal.
  Handles the Floor 9 full-heal + enemy clear.
- `updateFloorMechanics(dt)` — per-floor ticks (timer, surge, traps).
- `advanceAfterGear()` — routes to boon picker or directly to next floor
  depending on which floor was just completed.
- `offerBoonChoice(callback)` — sets `G.boonChoices` + `G._boonCallback`,
  switches screen to `'boon'`.
- `updateBoss(dt)` / `dropSkySpear()` — Wilhelm's AI. Phases driven by
  `b.phaseTriggered` advancing on 80/60/40/20% HP thresholds.

Data tables at top of script:
- `CLASSES` — fire/ice definitions, each with 10 spells
- `ENEMY_DATA` — 11 enemy types with stats
- `GEAR_POOL` — 15 gear items (rarity tiers)
- `BIOMES` — per-floor palette (sky/floor/wall/overlay)
- `BOONS` — 8 boons with `apply(p)` functions

## How to run

Open `obelisk.html` in any modern browser. No build step. Single file.

## User preferences observed in this session

- Wants visible, playable feedback quickly — camera and sprite visibility
  were the first functional concerns after the spec-alignment pass.
- Responds to screenshots of the running game as the source of truth over
  code walkthroughs.

---

# Part 2 — Complete Build Specification (PIONEERS_0.1)
...
| Spell loadout screen | Included |
| Between-floor boon system | Framework + 6–8 boons minimum |
| Gear drops + stats | Included |
| Relic system | Framework only (3–5 placeholder relics) |
| Combat feel (hit pause, shake, particles) | Required |
| Elite enemies with modifiers | Included |
