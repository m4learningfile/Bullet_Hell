...
function drawBackground(dt){
  ...
  const variant = game.mapVariant||0;
  ...
  else if(variant===0){g.addColorStop(0,'#04070e');g.addColorStop(0.5,'#091220');g.addColorStop(1,'#03060b');}
  else if(variant===1){g.addColorStop(0,'#04111c');g.addColorStop(0.5,'#0d2740');g.addColorStop(1,'#061522');}
  else if(variant===2){g.addColorStop(0,'#1a0608');g.addColorStop(0.5,'#3d1017');g.addColorStop(1,'#120509');}
  else if(variant===3){g.addColorStop(0,'#081207');g.addColorStop(0.5,'#163114');g.addColorStop(1,'#060e06');}
  else {g.addColorStop(0,'#0a0813');g.addColorStop(0.5,'#22153a');g.addColorStop(1,'#07060d');}
  ...
  if(!skullBG){
    ...
    if(variant===0){
      ctx.strokeStyle='rgba(110,180,255,0.16)';
      for(let x=A.x; x<=A.x+A.w; x+=48){ ctx.beginPath(); ctx.moveTo(x,A.y); ctx.lineTo(x,A.y+A.h); ctx.stroke(); }
      for(let y=A.y; y<=A.y+A.h; y+=48){ ctx.beginPath(); ctx.moveTo(A.x,y); ctx.lineTo(A.x+A.w,y); ctx.stroke(); }
    } else if(variant===1){
      ctx.strokeStyle='rgba(90,200,255,0.18)';
      for(let i=0;i<16;i++){ const x=A.x+i*(A.w/15); ctx.beginPath(); ctx.moveTo(x,A.y); ctx.lineTo(x+30,A.y+A.h); ctx.stroke(); }
    } else if(variant===2){
      ctx.strokeStyle='rgba(255,90,120,0.14)';
      for(let i=0;i<12;i++){ const y=A.y+i*(A.h/11); ctx.beginPath(); ctx.moveTo(A.x,y); ctx.lineTo(A.x+A.w,y+24*Math.sin(i)); ctx.stroke(); }
    } else if(variant===3){
      ctx.strokeStyle='rgba(120,255,120,0.14)';
      for(let i=0;i<22;i++){ const x=A.x+Math.random()*A.w, y=A.y+Math.random()*A.h; ctx.beginPath(); ctx.arc(x,y,2+((i%3)),0,Math.PI*2); ctx.stroke(); }
    } else {
      ctx.strokeStyle='rgba(200,120,255,0.14)';
      for(let i=0;i<14;i++){ ctx.beginPath(); ctx.arc(A.x+A.w/2,A.y+A.h/2,50+i*26+(Math.sin(game.bgT*2+i)*5),0,Math.PI*2); ctx.stroke(); }
    }
    ctx.restore();
  }

  if(game.skullFlash>0){ game.skullFlash=Math.max(0,game.skullFlash-dt); ctx.fillStyle='rgba(255,255,255,'+(0.25*game.skullFlash)+')'; ctx.fillRect(0,0,WORLD_W,WORLD_H); }
  for(let i=0;i<3;i++){
    const t=game.bgT+i*1.3, x=WORLD_W/2+Math.cos(t*0.3+i)*600, y=WORLD_H/2+Math.sin(t*0.4+i*2)*400;
    const rg=ctx.createRadialGradient(x,y,0,x,y,900);
    rg.addColorStop(0,hsl(h+i*40,60,16,0.55)); rg.addColorStop(1,hsl(h+i*40,60,10,0));
    ctx.fillStyle=rg; ctx.fillRect(0,0,WORLD_W,WORLD_H);
  }

  ctx.save();
  ctx.fillStyle='rgba(0,0,0,0.35)'; ctx.fillRect(A.x,A.y,A.w,A.h);
  ctx.strokeStyle=skullBG?'rgba(0,0,0,0.65)':hsl(h+180,60,60,0.75);
  ctx.lineWidth=3; ctx.shadowColor=hsl(h+180,80,60); ctx.shadowBlur=26;
  ctx.strokeRect(A.x,A.y,A.w,A.h);
  ctx.restore();

  if(game.mapPortals && game.mapPortals.length){
    for(const p of game.mapPortals){
      const ex = game.mapPortals.find(q=>q.side===p.exitSide);
      const hue = p.side==='top'?190:p.side==='right'?45:p.side==='bottom'?300:135;
      ctx.save();
      ctx.shadowColor=hsl(hue,90,65); ctx.shadowBlur=24;
      ctx.strokeStyle=hsl(hue,90,65); ctx.lineWidth=8;
      if(p.w>p.h){
      ...
      ctx.fillStyle=hsl(hue,90,65,0.16);
      ctx.fillRect(p.x, p.y, p.w, p.h);
      ctx.restore();
      if(ex){
        ctx.fillStyle='rgba(255,255,255,0.92)';
        ctx.font='700 12px Segoe UI';
        ctx.fillText('→ '+ex.side.toUpperCase(), p.cx-24, p.cy-12);
      }
    }
  }

  if(!skullBG && game.mapName){
    ctx.fillStyle='rgba(255,255,255,0.12)';
    ctx.font='700 18px Segoe UI';
    ctx.fillText(game.mapName, A.x+14, A.y-14);
  }
}
...
function drawEnemy(e){
  ...
  switch(e.type){
    ...
    case 'kiteenemy': {
      if(e.tail && e.tail.length>1){
        ctx.save(); ctx.rotate(0); ctx.shadowBlur=0; ctx.strokeStyle='rgba(120,255,210,0.55)'; ctx.lineWidth=10; ctx.lineCap='round';
        ctx.beginPath(); ctx.moveTo(e.tail[0].x-e.x, e.tail[0].y-e.y);
        for(let i=1;i<e.tail.length;i++) ctx.lineTo(e.tail[i].x-e.x, e.tail[i].y-e.y);
        ctx.stroke(); ctx.restore();
      }
      ctx.beginPath(); ctx.moveTo(e.r,0);ctx.lineTo(-e.r*0.2,-e.r*0.65);ctx.lineTo(-e.r*0.75,0);ctx.lineTo(-e.r*0.2,e.r*0.65);ctx.closePath();ctx.fill();ctx.stroke();
      break;
    }
    case 'healthpack': {
