...
function defaultSave() {
  return {
    ...
    disabledUpgrades: {},
    options: { masterVolume: 0.32, eliteEnemies: true, shootMode: 'hold', autoAim: false },
    adminMode: false,
    specialUnlocks: { kite:false, crescent:false },
    unlockProgress: { crescentBlockStreak:0 },
...
function loadSave() {
  try {
    ...
    return {
      ...
      specialUnlocks: Object.assign({},def.specialUnlocks,d.specialUnlocks||{}),
      unlockProgress: Object.assign({},def.unlockProgress,d.unlockProgress||{}),
...
const CLASS_INFO = {
  ...
  kite:{
    ...
    unlockHint:'Kill the Wandering Kite enemy.',
  },
  crescent:{
    ...
    skillDescription:'For 10 seconds your bullets transform into indigo crescents: piercing boomerangs that fly out to your cursor position and return. Only one volley can be active at a time, so you must wait for it to come back before firing again. Each hit deals 4× your bullet damage. After the form ends, the cooldown begins normally and can be reduced by Skill Cooldown boons.',
...
function isUnlocked(cls) {
  ...
  switch(cls) {
    ...
    case 'kite':     return !!saveData.specialUnlocks.kite;
    case 'crescent': return !!saveData.specialUnlocks.crescent;
...
const ENEMY_INFO = {
  kiteenemy:{ name:'WANDERING KITE', cost:'—', category:'Special', desc:'1% chance per wave on Normal or harder. 1000 HP, fixed move speed, huge harmless tail, despawns after about 30 seconds if not killed. Killing it unlocks the Kite class.' },
  ...
  healthpack:{name:'HEALTH PACK', cost:'—', category:'Support', desc:'Forest-green cross, 1 HP, stationary. Chance to appear once per wave based on difficulty (Easy 80% / Normal 50% / Hard 25% / Insane 0%). Killing it heals you for 10 HP.' },
};
...
const BOSS_INFO = {
  ...
  skull:{
    ...
    desc:'A dual-skull boss encounter. Two stationary skulls fire rotating bullet spreads. When one dies it sends out a lethal block-only shock ring for 5 damage.',
    tips:'Read the rotating spread rhythm. Kill one when you are ready to block the death ring.'
  },
};
